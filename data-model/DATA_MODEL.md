...
| Catalogue curation (read-only dataset untouched) | The generated `tour_packages.theme` and component titles ignore the place (a "Honeymoon" package in the temple town of Tirupati, a jazz cellar in Amritsar, a beach in inland Ooty). `backend/src/packagepro.ts → PLACE_THEME / misfitReason()` shows the theme the place is known for (name and description follow it) and leaves out lines that don't belong: nightlife in sacred towns, sea and backwaters inland, hot springs and railway pick-ups where there are none, repeated swap options. IDs and prices are unchanged |
