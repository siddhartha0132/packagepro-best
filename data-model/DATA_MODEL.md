...
| `app_guide_bookings` (`gbk_…`) | One row per guide per booked date. Live availability = `guide_availability.slots_available` − confirmed rows, so a confirmed trip blocks the next traveller. Capacity is re-checked inside the booking transaction |
