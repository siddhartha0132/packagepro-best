...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, ChevronRight, CircleAlert, Languages, MapPin, MessageCircle, Sparkles, Star } from "lucide-react";
...
export default function Home() {
  ...
  const swapHotel = trpc.trip.swapHotel.useMutation({ onSuccess: () => utils.trip.get.invalidate(), onError: error => toast.error(error.message) });
  const removeGuide = trpc.trip.removeGuide.useMutation({ onSuccess: () => utils.trip.get.invalidate(), onError: error => toast.error(error.message) });
  ...
  const loading = createTrip.isPending || autoBuild.isPending || selectFlight.isPending || selectHotel.isPending || swapHotel.isPending || removeGuide.isPending || swap.isPending || continuePackage.isPending || selectGuide.isPending || skipGuide.isPending || negotiate.isPending || goBack.isPending || confirm.isPending;
  ...
  function exportWhatsApp() {
    if (!trip) return;
    window.open(buildWhatsAppUrl(trip), "_blank", "noopener,noreferrer");
...
function CuratedPackages({ packages, recommendations, recommendedGuides, destinationInsight, selectedPackageId, onSelectPackage, lang, onChoose }: { packages: any[]; recommendations: any[]; recommendedGuides: any[]; destinationInsight?: any; selectedPackageId: string | null; onSelectPackage: (id: string | null) => void; lang: Lang; onChoose: (city: string, id: string) => void }) {
  ...
  return <div className="mt-10 border-t border-[#d8d7cd] pt-8">
    ...
    </div>{destinationInsight && <div className="mt-4 rounded-lg border border-[#d8d7cd] bg-[#fbfaf6] p-4"><div className="text-[10px] font-bold uppercase tracking-[.16em] text-[#286c62]">Destination context · {destinationInsight.city}</div><p className="mt-2 text-xs leading-5 text-[#68736c]">{destinationInsight.summary}</p><div className="mt-2 text-[10px] text-[#9ba19b]">Source: {destinationInsight.source}</div></div>}
...
function PlanActions({ saved, onSave, onShare, onWhatsApp, saveLabel, savedLabel, shareLabel, whatsappLabel }: { saved: boolean; onSave: () => void; onShare: () => void; onWhatsApp?: () => void; saveLabel: string; savedLabel: string; shareLabel: string; whatsappLabel: string }) {
  return <div className="grid grid-cols-3 gap-2"><Button variant="outline" className="border-[#d8d7cd] bg-white/60 text-xs" onClick={onSave}><Check className="mr-1 h-3.5 w-3.5" />{saved ? savedLabel : saveLabel}</Button><Button variant="outline" className="border-[#d8d7cd] bg-white/60 text-xs" onClick={onShare}>{shareLabel} <ArrowRight className="ml-1 h-3.5 w-3.5" /></Button>{onWhatsApp && <Button variant="outline" className="border-[#2f9b65] bg-white/60 text-xs text-[#207a4b] hover:bg-[#e9f7ee]" onClick={onWhatsApp}><MessageCircle className="mr-1 h-3.5 w-3.5" />{whatsappLabel}</Button>}</div>;
}
